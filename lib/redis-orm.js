  ...
  RedisORM = (function(_super) {
      ...
      */
      ...
      rorm_save: function(model, expire, callback) {
        ...
        return this.rorm_deepSaveAtts(model, function(err, key) {
          ...
          if (0 < seconds) {
            _this.rorm_expire(model, seconds);
          }
          if (callback != null) {
      ...
      rorm_expire: function(model, seconds, callback) {
        var expire, key;
        expire = parseInt(seconds);
        key = this.rorm_refKeyForModel(model);
        if (0 < expire) {
          this.rorm_redis().expire(key, expire, callback);
        } else {
    ...
    RedisORM.addTheseToInstance = {
      ...
      */
      ...
      expire: function(secondsTillExpiration, callback) {
        return this.rorm_class().rorm_expire(this, secondsTillExpiration, callback);
      },
