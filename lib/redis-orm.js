    ...
    RedisORM.addTheseToClass = {
      ...
      */
      ...
      rorm_deepSaveHashValues: function(hash) {
        var h, k, v, val, _i, _len;
        h = {};
        for (v = _i = 0, _len = hash.length; _i < _len; v = ++_i) {
          k = hash[v];
          val = (typeof v.save === "function" ? v.save() : void 0) || v;
          h[k] = val;
        }
        return h;
      },
      ...
      */
      ...
      rorm_refKeyForModel: function(model) {
        return this.rorm_prefix + model.constructor.name + this.rorm_delim + model.id;
      },
      rorm_attsForModel: function(model) {
        var _this = this;
        return objSubset(model, function(key, value) {
          if (key.startsWith('rorm_')) {
            return false;
          }
          if (typeof value === _this.rorm_TYPE_FUNCTION) {
            return false;
          }
          return true;
        });
      ...
      rorm_deepSaveRefs: function(atts) {
        ...
        return objMap(atts, function(key, val) {
          var mappedValue;
          if (val instanceof Array) {
            mappedValue = _this.rorm_deepSaveArrayValues(val);
          } else {
            if (val.save != null) {
              console.log("SAVINGSAVINGSAVINGSAVING REF", val.id);
            }
    ...
    RedisORM.addTheseToInstance = {
      ...
      */
      ...
      save: function() {
        ...
        this.id || (this.id = 'generate-a-unique-id-already');
